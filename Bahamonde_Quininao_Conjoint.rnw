...
\autoref{fig:amce:plot} suggests that respondents systematically preferred hypothetical candidates who supported democratic policies. Authoritarian candidates that can rule without Congress, or political systems in which there is controlled mass media, or where citizens are not allowed to vote, run for office or associate with others, are systematically rejected by the nationally represented pool of respondents. These analyses are not surprising as they conform with our theoretical priors, i.e. the United States has been (traditionally) considered a strong democracy. 

While classic conjoint analyses provide consistent causal estimates, they unfortunately overlook respondent's preferences. As \autoref{fig:amce:plot} suggests, the analyst can causally \emph{describe} aggregate behaviors but cannot observe the relationship between the respondent's preferences and attribute choices. We argue that the descriptive nature of the classic conjoint design (for instance, via the AMCE) might (wrongly) suggest that democratic values scored high in the United States. After all, \autoref{fig:amce:plot} strongly indicates that non-democratic candidate attributes are systematically rejected (i.e. all coefficients are negative). And as a consequence, that might imply that the intention to sell the vote should be low. %However, it should be noted that these data were collected during 2016, year in which Donald Trump won the U.S. presidency. 


Next section introduces support vector machine methods that allow analyzing conjoint data considering subject preferences and/or attitudes. The ability to introduce covariates, such as the intention to sell the vote,  might shed some light about how healthy or broken democratic values were at the time were the data were collected (which coincides with the campaign period that gave Donald Trump the U.S. presidency). We claim this is an exceptional opportunity to study democracy, and particularly, the democratic attitudes that when broken predict vote selling.
