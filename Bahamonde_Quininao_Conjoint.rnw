...
\paragraph{Stating the problem} Consider the following classification task: estimate a function $f:\R^J\rightarrow\{-1,1\}$ using a system of {\color{red}input-output training data pairs}\todo{explain}. The $f$ function should be able to map every participant attitudes toward the five democracy attributes and classify those attitudes within a common numeric space. 
...
and from now on, the intercept $b$ will be ignored because a function of the type $f_i(\mathbf z_k)=\text{sgn}(\w_i\cdot\mathbf z^i_k)$ is mathematically sufficient \todo{necesitamos probar esto? suena como arbitrario?}. 
...
where the regularization constant $C>0$ determines the trade-off between the empirical error and the {\color{red}complexity term}\todo{explayar}. The optimization problem was solved using the \texttt{sklearn} library in \texttt{Python}. We also make available in the Appendix an \texttt{R} routine that directly solves the optimization problem by using the {\color{red}steepest descent approach}\todo{explayar}. Since the optimization problem is convex, algorithms converge to a unique solution.
