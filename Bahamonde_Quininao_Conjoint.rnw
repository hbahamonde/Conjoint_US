...
\toprule
\textbf{Dalh's Polyarchy Dimension} & \textbf{Dalh's Requirements for a Democracy} & \textbf{Experimental Operationalization for Conjoint Design} \\
\midrule
...
% Our conjoint
Conjoint designs are suitable tools to ``determine which components of the manipulation produce the observed effect'' \parencite[2]{Hainmueller2014a}. \autoref{tab:dim} specifies three general dimensions that should be satisfied for a country to be considered a democracy \parencite{Dahl1971}. Every dimension has a number of requirements. In order to operationalize \textcite{Dahl1971}'s requirements, we devised for the conjoint experiment five attributes that reflected these requirements: (1) \emph{media can confront the government}, (2) \emph{president cannot rule without congress}, (3) \emph{citizens can vote in the next two elections}, (4) \emph{citizens can run for office for the next two elections} and (5) \emph{citizens can associate with others and form groups}. Given that conjoint designs are able ``to identify the causal effects of various components of a treatment in survey experiments'' \parencite[2]{Hainmueller2014a}, we claim that this is an appropriate tool to shed some light on the multi-causal study of clientelism. 
...
\autoref{fig:amce:plot} suggests that respondents systematically preferred hypothetical candidates who supported democratic policies. Authoritarian candidates that can rule without Congress, or political systems in which there is controlled mass media, or where citizens are not allowed to vote, run for office or associate with others, are systematically rejected by the nationally represented pool of respondents. These analyses are not surprising as they conform with our theoretical priors, i.e. the United States has been (traditionally) considered a strong democracy. However, it should be noted that these data were collected during 2016, year in which Donald Trump won the U.S. presidency. 

Since traditional conjoint analyses do not allow for studying relationships between subject attitudes and attribute choices, we might (wrongly) conclude that democratic values scored high in the United States. Hence, the intention to sell the vote should be low. In fact, the \href{https://www.vanderbilt.edu/lapop/usa/2010_United_States_Questionnaire.pdf}{LAPOP} 2010 survey wave for the United States suggests that \Sexpr{percentage.never}\% of respondents has never been asked to sell the vote. Again, signaling good democratic values in 2016. Next section introduces support vector machines that allow analyzing conjoint data considering subject attitudes (such as intention to sell the vote). By doing so we hope to better understand how healthy or broken democratic values were during the campaign period that gave Donald Trump the U.S. presidency.


\section{Introducing Support Vector Machines Techniques}\hypertarget{our.cj.svm}{}
